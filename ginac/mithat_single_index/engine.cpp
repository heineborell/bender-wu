...
std::vector<ex> energy(const std::vector<ex> &f_n_x0, ex &omega) {
  ...
  int Nk{nu + 3 * lmax + 2};
  std::vector<ex> A((lmax + 1) * (nu + 3 * lmax + 3));
  A.data()[0 * Nk + nu] = 1; // Normalization
  ...
  for (int l{0}; l <= lmax; l += lstep) {
    ...
    if (l > 0) {
      ...
      for (int k{nu + 3 * l}; k > nu; --k) {
        A.data()[0 * Nk + k] = 0; // A[0][k]=0 as Hermite is bounded with nu
        sum = numeric(k + 2) * numeric(k + 1) * A.data()[l * Nk + (k + 2)];
        ...
        for (int n{1}; n <= l / 2; ++n) {
          sum += 2 * E.data()[n] * A.data()[(l - 2 * n) * Nk + k];
        }
        A.data()[l * Nk + k] = evalf(sum / (omega * numeric(2 * (k - nu))));
        ...
        if (l % 2 == 0) {
          ...
          for (int n{1}; n <= std::min(l, nu - 2); ++n) {
            sum += f_n_x0.data()[n] * A.data()[(l - n) * Nk + (nu - n - 2)];
          }
